...
class BEVDet(CenterPoint):
    ...
    @force_fp32()
    def bev_encoder(self, x, **kwargs):
        if hasattr(self, "prior_fusion_module"):
            prior_voxels = kwargs["prior_voxels"]
            prior_voxels_coords = kwargs["prior_voxels_coords"]
            # x: (bs, c, z, h, w)
            x = x.permute(0, 1, 3, 4, 2)
            if getattr(self, "use_prior_only", False):
                x = torch.zeros_like(x)
            fused_bev_feats = self.prior_fusion_module(x, prior_voxels, prior_voxels_coords) # (bs, bev_c, bev_h, bev_w, bev_z)
            x = fused_bev_feats.permute(0, 1, 4, 2, 3)
    ...
    def forward_test(self,
        ...
        if not isinstance(img_inputs[0][0], list):
            ...
            if "prior_voxels" in kwargs:
                prior_voxels = kwargs["prior_voxels"][0].data[0]
                if isinstance(prior_voxels, list):
                    prior_voxels = [p.cuda() for p in prior_voxels]
                else:
                    prior_voxels = prior_voxels.cuda()
                kwargs['prior_voxels'] = prior_voxels
            if "prior_voxels_coords" in kwargs:
                prior_voxels_coords = kwargs["prior_voxels_coords"][0].data[0]
                if isinstance(prior_voxels_coords, list):
                    prior_voxels_coords = [p.cuda() for p in prior_voxels_coords]
                else:
                    prior_voxels_coords = prior_voxels_coords.cuda()
                kwargs['prior_voxels_coords'] = prior_voxels_coords
            
            return self.simple_test(points[0], img_metas[0].data[0], img_inputs[0],
                                    **kwargs)
        else:
            return self.aug_test(None, img_metas[0], img_inputs[0].data, **kwargs)
            ...
            for adj_id in range(self.num_frame-2):
                bev_feat_list[adj_id] = \
                    self.shift_feature(bev_feat_list[adj_id],
                                       ...
                                       bda)
        bev_feat = torch.cat(bev_feat_list, dim=1)
        x = self.bev_encoder(bev_feat, **kwargs)
