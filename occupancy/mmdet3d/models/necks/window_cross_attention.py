...
class WindowCrossAttention(nn.Module):
    def __init__(self,
                 ...
                 init_cfg=None):
        super().__init__()
        if bev_h % num_bev_win != 0:
            raise ValueError(f'bev_h must be divisible by num_bev_win, '
                             f'but got {bev_h} and {num_bev_win}')
        if bev_w % num_bev_win != 0:
            raise ValueError(f'bev_w must be divisible by num_bev_win, '
                             f'but got {bev_w} and {num_bev_win}')

        self.num_bev_win = num_bev_win
        self.bev_h = bev_h
        self.bev_w = bev_w
        self.bev_win_size= self.bev_h // self.num_bev_win
        # assert self.bev_win_size_h == self.bev_win_size_w
        self.window_size = self.bev_win_size
    ...
    def forward(self,
        ...
        query = query.reshape(bs, self.bev_h, self.bev_w, C)

        _, _, C = key.size()
        # [batch size, in channels, height, width]
        key = key.reshape(bs, self.bev_h, self.bev_w, C)
        ...
        bs, channels, height, width = query.shape
        # Make patches
