...
class NuScenesDatasetOccpancy(NuScenesDataset):
    ...
    def evaluate(self, occ_results, runner=None, show_dir=None, **eval_kwargs):
        ...
        print('\nEvaluating without camera masks...')
        new_occ_eval_metrics = Metric_mIoU(
            num_classes=18,
            use_lidar_mask=False,
            use_image_mask=True)
        for index, occ_pred in enumerate(tqdm(occ_results)):
            ...
            # occ_pred = occ_pred
            new_occ_eval_metrics.add_batch(occ_pred, gt_semantics, mask_lidar, mask_camera)

        new_occ_eval_metrics.count_miou()
