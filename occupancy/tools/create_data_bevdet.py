...
def nuscenes_data_prep(root_path, info_prefix, version, max_sweeps=10, prior_split=False):
    ...
    nuscenes_converter.create_nuscenes_infos(
        root_path, info_prefix, version=version, max_sweeps=max_sweeps, prior_split=prior_split)
    ...
    for set in ['train', 'val']:
        ...
        for id in range(len(dataset['infos'])):
            ...
            dataset['infos'][id]['occ_path'] = \
                './data/nuscenes/occ3d-nus/gts/%s/%s'%(scene['name'], info['token'])
...
def parse_args():
    ...
    parser.add_argument(
        '--extra-tag',
        default='bevdetv2-nuscenes-priorsplit')
...
if __name__ == '__main__':
    ...
    prior_split = "priorsplit" in extra_tag
    if prior_split:
        print("using priorsplit!")
    nuscenes_data_prep(
        root_path=root_path,
