...
prior_pc_range = [-40, -40, -2.0, 40, 40, 6.0]
...
model = dict(
    ...
    prior_fuse_cfg=dict(
        fusion_module_cfg=dict(
            type="PriorFusion3D_voxel",
            prior_pc_range=prior_pc_range,
            prior_voxel_size=prior_voxel_size,
...
train_pipeline = [
    ...
    dict(
        type='Collect3D', keys=['img_inputs', 'gt_depth', 'voxel_semantics',
                                'mask_lidar','mask_camera', 'prior_voxels', 'prior_voxels_coords'])
]
    ...
    dict(
        ...
        transforms=[
            dict(
                ...
                with_label=False),
            dict(type='Collect3D', keys=['points', 'img_inputs', 'prior_voxels', 'prior_voxels_coords'])
        ])
]
...
test_data_config = dict(
    ...
    ann_file=data_root + 'bevdetv3-nuscenes-priorsplit_infos_val.pkl',
...
data = dict(
    ...
    train=dict(
        data_root=data_root,
        ann_file=data_root + 'bevdetv3-nuscenes-priorsplit_infos_train.pkl',
...
checkpoint_config = dict(interval=8, max_keep_ckpts=2)
