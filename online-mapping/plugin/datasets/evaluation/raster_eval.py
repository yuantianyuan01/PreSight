...
class RasterEvaluate(object):
    ...
    def __init__(self, dataset_cfg: Config, n_workers: int=4):
    ...
    def evaluate(self, 
        ...
        for token, gt in self.gts.items():
            gts.append(gt)
            if token in results:
                pred = torch.from_numpy(results[token]['predict_mask'])
            else:
                pred = torch.zeros((len(self.cat2id), 
                    self.canvas_size[1], self.canvas_size[0]), dtype=torch.bool)
