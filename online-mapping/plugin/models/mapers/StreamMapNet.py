...
class StreamMapNet(BaseMapper):
    ...
    def __init__(self,
        ...
        if self.streaming_bev:
            ...
            x = torch.linspace(xmin, xmax, bev_w).view(1, -1).expand(bev_h, bev_w)
            y = torch.linspace(ymin, ymax, bev_h).view(-1, 1).expand(bev_h, bev_w)
    ...
    def forward_train(self, img, vectors, points=None, prior_voxels=None, 
        ...
        gts, img, img_metas, valid_idx = self.batch_data(
            vectors, img, img_metas, img.device)
