...
import argparse
from nusc_presight_split import VAL, PRIOR, POSE_FAIL_SCENES, MAP_FAIL_SCENES
...
def create_nuscenes_infos_map(root_path,
    ...
    if prior_split:
        scenes = nusc.scene
        FAIL_SCENES = MAP_FAIL_SCENES + POSE_FAIL_SCENES
        train_scenes = [
            s["name"] for s in scenes if s["name"] not in VAL + PRIOR + FAIL_SCENES
        ]
        val_scenes = [
            s["name"] for s in scenes if s["name"] in VAL and s["name"] not in FAIL_SCENES
        ]
        prior_scenes = [
            s["name"] for s in scenes if s["name"] in PRIOR and s["name"] not in FAIL_SCENES
        ]
        assert len(set(train_scenes + val_scenes + prior_scenes)) == len(train_scenes + val_scenes + prior_scenes)
        assert len(set(train_scenes + val_scenes + prior_scenes)) == len(set(splits.train + splits.val) - set(FAIL_SCENES))
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        if scene_name in MAP_FAIL_SCENES: continue
        info = {
    ...
    if test:
        ...
        # for training set
        if prior_split:
            info_path = osp.join(dest_path, f'{info_prefix}_map_infos_train_priorsplit.pkl')
        else:
            info_path = osp.join(dest_path, f'{info_prefix}_map_infos_train.pkl')
        print(f'saving training set {len(train_samples)} samples to {info_path}')
        mmcv.dump(train_samples, info_path)

        # for val set
        if prior_split:
            info_path = osp.join(dest_path, f'{info_prefix}_map_infos_val_priorsplit.pkl')
        else:
            info_path = osp.join(dest_path, f'{info_prefix}_map_infos_val.pkl')
        print(f'saving validation set of {len(val_samples)} samples to {info_path}')
...
if __name__ == '__main__':
    ...
    create_nuscenes_infos_map(root_path=args.data_root, version=args.version, prior_split=True)