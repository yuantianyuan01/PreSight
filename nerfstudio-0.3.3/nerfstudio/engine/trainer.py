...
class TrainerConfig(ExperimentConfig):
    ...
    init_grad_scale: float = 2.**10
    update_grad_scaler: bool = False
...
class Trainer:
    ...
    def __init__(self, config: TrainerConfig, local_rank: int = 0, world_size: int = 1) -> None:
        ...
        # optimizers
        self.grad_scaler = GradScaler(init_scale=self.config.init_grad_scale, enabled=self.use_grad_scaler)
    ...
    def setup(self, test_mode: Literal["test", "val", "inference"] = "val") -> None:
        ...
        banner_messages.append(f"Exp: {self.config.method_name}/{self.config.experiment_name}")
    ...
    def train_iteration(self, step: int) -> TRAIN_INTERATION_OUTPUT:
        ...
        for _ in range(self.gradient_accumulation_steps):
            ...
            with profiler.time_function('loss.backward'):
                self.grad_scaler.scale(loss).backward()  # type: ignore
        
        if self.config.update_grad_scaler:
        ...
        if self.config.log_gradients:
            total_grad = 0
            metrics_dict[f"Gradients/grad_scale"] = self.grad_scaler.get_scale()
        ...
        # profiler.flush_profiler(cfg.LoggingConfig())
    ...
    def eval_iteration(self, step: int) -> None:
        ...
        if self.pipeline.datamanager.eval_batch_dataset and step_check(step, self.config.steps_per_eval_batch):
            _, eval_loss_dict, eval_metrics_dict = self.pipeline.get_eval_loss_dict(step=step)
            eval_loss = functools.reduce(torch.add, eval_loss_dict.values())
            writer.put_scalar(name="Eval Loss", scalar=eval_loss, step=step)
            # writer.put_dict(name="Eval Loss Dict", scalar_dict=eval_loss_dict, step=step)
            writer.put_dict(name="Eval Metrics Dict", scalar_dict={**eval_metrics_dict, **eval_loss_dict}, step=step)

        # one eval image
        if self.pipeline.datamanager.eval_image_dataset and step_check(step, self.config.steps_per_eval_image):
        ...
        # all eval images
        if self.pipeline.datamanager.eval_image_dataset and step_check(step, self.config.steps_per_eval_all_images):
