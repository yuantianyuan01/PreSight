...
class PropNetDensityField(Field):
    ...
    def get_density(self, ray_samples: RaySamples) -> Tuple[Tensor, None]:
        ...
        return self.density_fn(positions), None
    ...
    def density_fn(self, positions: Float[Tensor, "*bs 3"]) -> Float[Tensor, "*bs 1"]:
        ...
        if not self.use_linear:
            density_before_activation = (
                self.mlp_base(positions_flat).view(*positions.shape[:-1], -1).to(positions.dtype)
            )
        else:
            x = self.encoding(positions_flat).to(positions.dtype)
            density_before_activation = self.linear(x).view(*positions.shape[:-1], -1)
