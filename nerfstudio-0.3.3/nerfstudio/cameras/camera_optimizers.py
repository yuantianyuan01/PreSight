...
class CameraOptimizerConfig(InstantiateConfig):
    ...
    mode: Literal["off", "SO3xR3", "SE3", "xyz", "z"] = "off"
    """Pose optimization strategy to use. If enabled, we recommend SO3xR3."""

    scale: float = 1.0
    """Pose tranformation scale"""
...
class CameraOptimizer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        elif self.config.mode == "xyz":
            self.pose_adjustment = torch.nn.Parameter(torch.zeros((num_cameras, 3), device=device))
        elif self.config.mode == "z":
            self.pose_adjustment = torch.nn.Parameter(torch.zeros((num_cameras, 1), device=device))
        else:
    ...
    def forward(
        ...
        if self.config.mode == "off":
            ...
            transform = torch.eye(4, device=self.device)[None, :3, :4].repeat(indices.shape[0], 1, 1)
            transform[:, :3, 3] = self.pose_adjustment[indices, :] * self.config.scale
            outputs.append(transform)
        elif self.config.mode == "z":
            transform = torch.eye(4, device=self.device)[None, :3, :4].repeat(indices.shape[0], 1, 1)
            transform[:, 2:3, 3] = self.pose_adjustment[indices, :] * self.config.scale
            outputs.append(transform)
        else:
            assert_never(self.config.mode)
