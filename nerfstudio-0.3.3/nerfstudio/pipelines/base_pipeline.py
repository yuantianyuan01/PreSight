...
class VanillaPipeline(Pipeline):
    ...
    def get_train_loss_dict(self, step: int):
        ...
        Args:
            step: current iteration step to update sampler if using DDP (distributed)
        """
        with profiler.time_function('next_train'):
            ray_bundle, batch = self.datamanager.next_train(step)
        with profiler.time_function('model'):
            model_outputs = self._model(ray_bundle)  # train distributed data parallel model if world_size > 1
        with profiler.time_function('get_metrics_dict'):
            metrics_dict = self.model.get_metrics_dict(model_outputs, batch)
    ...
    def get_param_groups(self) -> Dict[str, List[Parameter]]:
        ...
        total_param_groups = {**datamanager_params, **model_params}
        assert len(total_param_groups) == len(datamanager_params) + len(model_params)
        return total_param_groups
